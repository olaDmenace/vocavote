...
- Most schema lives **on the remote project**, not in the repo: 21 migrations have been applied via MCP but only two (the `20260625*` files under `supabase/migrations/`) are checked in — the other 19 plus `supabase/seed.sql` (seeds demo users incl. `auth.identities` + zeroed token columns) are the rest. Use `mcp__supabase__list_tables` / `list_migrations` to inspect the live schema. When applying new schema changes, prefer the MCP `apply_migration` tool (or local `supabase` CLI) over ad-hoc SQL, and follow the timestamped migration names in PRD §11.
