...
- Schema currently lives **on the remote project**, not in the repo: there is no `supabase/migrations/` directory yet, only `supabase/seed.sql` (seeds demo users incl. `auth.identities` + zeroed token columns). Use `mcp__supabase__list_tables` / `list_migrations` to inspect the live schema. When applying new schema changes, prefer the MCP `apply_migration` tool (or local `supabase` CLI) over ad-hoc SQL, and follow the timestamped migration names in PRD §11.
